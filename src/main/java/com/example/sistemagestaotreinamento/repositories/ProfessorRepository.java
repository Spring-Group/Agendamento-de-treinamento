...
public interface ProfessorRepository extends JpaRepository<Professor, Integer> {
    ...
    @Query("SELECT p FROM Professor p LEFT JOIN FETCH p.cursos WHERE p.id = :professorId")
    Professor findProfessorWithCursosById(int professorId);

    @Query("SELECT p FROM Professor p LEFT JOIN FETCH p.agendamentos WHERE p.id = :professorId")
